...
實際 token 定義位於 `src/styles/tokens.css`（CSS variables）與 `src/styles/_theme.scss`（Bootstrap SCSS overrides）。命名規格：`kind-role-(state)?` kebab-case。

| 類別 | Token |
|---|---|
| 背景 | `--bg-app` / `--bg-surface` / `--bg-surface-alt` / `--bg-elevated` |
| 邊框 | `--border-subtle` / `--border-strong` |
| 文字 | `--text-primary` / `--text-secondary` / `--text-muted` |
| Accent | `--accent-primary` / `--accent-secondary` / `--accent-hover` / `--accent-pressed`；`--accent` 為 `--accent-primary` 別名 |
| 狀態色 | `--color-success` / `--color-warning` / `--color-danger` / `--color-info` |
| 字型 | `--font-family-sans`（Inter 為首） / `--font-family-mono`（JetBrains Mono 為首） |
| 字級 | `--font-size-xs` / `-sm` / `-body` / `-lg` / `-xl` / `-display` |
| 字重 | `--font-weight-regular` / `-medium` / `-semibold` / `-bold` |
| 行高 | `--line-height-tight` / `-base` / `-relaxed` |
| 間距 | `--space-1` ~ `--space-8`（4px 基準） |
| 圓角 | `--radius-sm` / `-md` / `-lg` / `-pill` |
| Shadow | `--shadow-sm` / `-md` / `-elevated` / `-glow` |

入口檔：`src/styles/index.scss` 依序 `@import` tokens.css → `_theme.scss` → global.css，由 `main.tsx` 統一載入。設計系統視覺參考頁：`src/pages/_Styleguide.tsx`。

### 雙重來源維護備註（重要）

SCSS 編譯期看不到 CSS variables，因此 `_theme.scss` 中覆寫 Bootstrap 變數時 **必須直接寫色票字面值**，不能 `$primary: var(--accent-primary)`。改色時 `tokens.css` 與 `_theme.scss` 兩邊都要同步更新對應數值，否則會出現「Bootstrap 元件用一套色、自訂元件用另一套色」的視覺漂移。
